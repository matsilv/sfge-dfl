...
def print_acc(metrics, prefix):
    prefix += '_'

    print(f"Evaluation:: Loss: {metrics[prefix + 'loss']:.4f}, "
          f"Perfect acc: {metrics[prefix + 'perfect_accuracy']:.4f}, "
          f"Solution MSE: {metrics[prefix + MSE_SOL_STR]:.4f}, "
          f"Relative regret: {metrics[prefix + REL_REGRET_STR]:.4f}, "
          f"Feasible solutions relative regret: {metrics[prefix + FEAS_SOL_REL_REGRET_STR]:.4f}, "
          f"Infeasibility ratio: {metrics[prefix + NUM_INFEAS_SOL_STR]:.4f}")
